...
async function getProducts(page) {
    ...
    try {
        ...
        await axios.get(`http://16.171.155.148:3000/expense/get-expense?page=${page}&&date=${date}&&month=${month}&&year=${year}&&numRows=${numRows}`, { headers: { "Authorization": token } })
    ...
    try {
        ...
        deleteButton.onclick = async () => {
            try {
                let res;
                if (name === null) {
                    res = await axios.post(`http://16.171.155.148:3000/expense/delete-salary/${id}`, '', { headers: { "Authorization": token } });
                }
                else {
                    res = await axios.post(`http://16.171.155.148:3000/expense/delete-expense/${id}`, '', { headers: { "Authorization": token } });
                }
...
window.addEventListener('DOMContentLoaded', async () => {
    try {
        ...
        const response = await axios.get('http://16.171.155.148:3000/purchase/premiumOrNot', { headers: { "Authorization": token } });
        ...
        if (isPremium === 'true') {
            ...
            boardButton.onclick = async function (e) {
                ...
                const res = await axios.get('http://16.171.155.148:3000/premium/getLeaderboard', { headers: { "Authorization": token } });
...
document.getElementById('razorpayBtn').onclick = async function (e) {
    try {
        e.preventDefault();
        const token = localStorage.getItem('token');
        const response = await axios.get('http://16.171.155.148:3000/purchase/premiumMembership', { headers: { "Authorization": token } });
        ...
        var options = {
            ...
            "handler": async function (response) {
                await axios.post('http://16.171.155.148:3000/purchase/updateTransactionStatus', {
...
reportButton.onclick = async (e) => {
    try {
        ...
        const isPremium = response.data.isPremium;
        if (isPremium === 'true') {
            await axios.get('http://16.171.155.148:3000/users/download', { headers: { "Authorization": token } })
                .then((response) => {
